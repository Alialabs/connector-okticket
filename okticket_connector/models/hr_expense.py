...
_payment_method_selection = [
    ("efectivo", "Cash"),
    ("tarjeta", "Business card"),
    ("cheque", "Bank check"),
    ("tarjeta-gas", "Fuel card"),
...
class HrExpense(models.Model):
    ...
    okticket_deleted = fields.Boolean(string="Deleted in Okticket", default=False)

    okticket_response = fields.Text(string="Okticket Response")
    okticket_report_id = fields.Char(
        string="OkTicket Report ID",
        readonly=True,
        copy=False,
        help="Identifier of the OkTicket report (expense sheet) this expense"
        " belongs to in OkTicket",
    )
    okticket_report_name = fields.Char(
        string="OkTicket Report Name",
        readonly=True,
        copy=False,
        help="Name of the OkTicket report (expense sheet) this expense"
        " belongs to in OkTicket. Useful to filter and group expenses by"
    ...
    def _remove_deleted_draft_expenses(self):
        expenses_to_unlink = self.filtered(
            lambda exp: exp.okticket_deleted and exp.state == "draft"
        )
    ...
    def _check_unlink_conditions(self):
        for expense in self:
            if expense.state != "draft":
                raise UserError(
                    _(
                        "Deleting expenses in a state different from draft"
                        " is not allowed."
                    )
                )
    ...
    def _generate_unlink_log_message(self):
        user_info = f"User {self.env.user.name} [ID: {self.env.uid}]"
        expense_info = " | ".join(
            [f"ID: {exp.id} - OKTICKET_ID: {exp.okticket_expense_id}" for exp in self]
        )
