...
class HrExpenseBatchImporter(Component):
    ...
    def _pick_single_tax(self, taxes, product=None):
        ...
        without a single error in the log: 28 of 1236 expenses on the reference
        database, base equal to total and 0.00 of tax.
        ...
        * **Base taxes only.** A localisation that ships fiscal positions
          ...
          twelve candidates down to three (goods, investment goods, services).
        * **Matching scope.** ``tax_scope`` tells goods from services; an empty
        ...
        That last criterion is also the reason the per-product mapping exists:
        the connector types *every* expense product as a service, so a rate that
        is really goods cannot be expressed here and has to be stated on the
        product.
        ...
        if len(taxes) > 1 and product:
            scoped = taxes.filtered(
                lambda t: not t.tax_scope or t.tax_scope == product.type)
            if scoped:
                taxes = scoped
    ...
    def _resolve_record_taxes(self, record, company_id, product=None):
        ...
        **A receipt that declares no rate gets no tax.** The expense products
        are created with the chart's default purchase tax -- in a Spanish chart
        that is the 21% of *goods* -- so falling back to it invented both the
        rate and its nature on receipts that never claimed any VAT. Measured on
        the reference company: 488 of 733 expenses carry no breakdown at all,
        and every one of them was booking 21% of deductible input VAT on goods,
        taxis and hotels included. If OkTicket does not say there is VAT, the
        expense arrives without it.

        :return: list of account.tax ids -- empty to import with no tax -- or
            None to keep the product defaults.
        """
        raw_taxes = record.get('taxes')
        if not company_id:
            return None
        if not raw_taxes or not isinstance(raw_taxes, (list, tuple)):
            # OkTicket no declara impuestos para este gasto: no se inventa ninguno.
            return []
        ...
        for entry in raw_taxes:
            ...
            if rate not in rates:
                rates.append(rate)
        if not rates:
            # Todas las entradas con base cero: el ticket no declara IVA.
            return []
        if len(rates) > 1:
            msg = _('Expense %s reports several tax rates (%s) but an Odoo expense '
                    'holds a single taxable base; imported with no tax. Review '
                    'this expense manually.') % (
            ...
            _logger.warning(msg)
            # Ninguna respuesta es fiel con una sola base imponible, y quedarse
            # con los impuestos por defecto del producto es inventar uno de los
            # tipos. Entra sin impuesto y con el aviso.
            return []
        ...
        if not candidates:
            # El plan contable no tiene ningun impuesto de compra a ese tipo: el
            # 5 % de un aparcamiento, por ejemplo. Antes se quedaba con los
            # impuestos por defecto del producto -- el 21 % de bienes -- que es
            # inventarse un tipo distinto del que trae el recibo, y ademas solo
            # quedaba constancia en el log del servidor.
        ...
        if product is not None:
            ...
            #    for this rate. Explicit configuration always wins.
            mapped = product.product_tmpl_id.okticket_mapped_tax(
                rates[0], company_id)
            if mapped:
                return mapped.ids
            # El producto declara impuestos para otros tipos pero no para este:
            # el recibo trae un tipo que su categoria no admite -- un peaje al
            # 10 %, un repostaje al 0 % -- casi siempre un error de captura. No
            # se cae a los pasos siguientes porque acabarian poniendo el
            # impuesto por defecto del producto, que es inventarselo.
            declarados = product.product_tmpl_id.okticket_declared_rates(company_id)
            if declarados:
                msg = _('Expense %s reports %g%% but the category of product "%s" '
                    ...
                    ', '.join('%g%%' % r for r in sorted(declarados)))
                self.env['log.event'].add_event({
                    'backend_id': self.backend_record.id,
                    'type': 'warning',
                    'msg': msg,
        ...
        own = candidates & product.supplier_taxes_id \
            if product is not None else self.env['account.tax']
        # 3) Structural disambiguation.
        picked = self._pick_single_tax(own, product) \
            or self._pick_single_tax(candidates, product)
        if picked:
            return picked.ids
        # 4) Ambiguous: import the expense with no tax rather than with an
        #    invented one, and say which candidates could not be told apart.
        #    Returning an empty list is not the same as returning None -- None
        #    means "no usable rate reported", and the caller keeps the product
        #    defaults for that.
    ...
    def amount(self, record):
        if record.get('amount') is None:
            # Never import a silent 0.00 expense: leaving ``total_amount`` out
            # makes the required-fields check skip and log the record instead
            # of letting it reach accounting with a wrong amount.
            return
        return {
            # 'unit_amount' y no 'price_unit': el campo se renombro en 17.0.
            # Medido en esta version: con esta pareja el desglose de IVA llega
            # correcto (908 de 908 gastos con base + cuota = total), asi que el
            # defecto que obligo a escribir 'total_amount_currency' en 18.0 no
            # se reproduce aqui.
            'unit_amount': 0.0,  # Para hacer visibles los impuestos en la interfaz
    ...
    def run(self, filters=None, options=None):
        ...
        # 'total_amount' is required so an expense whose payload carries no
        # amount is skipped and logged instead of imported as 0.00.
        required_fields = ['product_id', 'employee_id', 'company_id', 'total_amount']
