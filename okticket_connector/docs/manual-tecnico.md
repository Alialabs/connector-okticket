...
<h1 align="center">Conector OkTicket para Odoo 16 · Manual técnico</h1>

<p align="center">
  Instalación, dependencias, configuración del conector y planificación de las sincronizaciones.<br>
  <sub>Módulos <b>16.0.1.1.x</b> · Odoo <b>16.0</b> · Licencia <b>AGPL-3</b> · Rama <code>16.0</code></sub>
</p>

> 📄 **Versión maquetada:** [descargar PDF](https://gitlab.alialabs.com/odoo/connector-okticket/-/raw/16.0-fix-okticket_connector/okticket_connector/docs/manual-tecnico.pdf?inline=false)
...
- Una instancia de **Odoo 16.0** (Community o Enterprise) sobre PostgreSQL.
...
El repositorio mantiene una rama por versión de Odoo. Para 16.0:

```bash
git clone --branch 16.0 https://github.com/Alialabs/connector-okticket.git
```
...
Ambos repositorios en su rama `16.0`:
...
| Ámbito | `*` | |
| Versión | `1.0 y superior` | Campo obligatorio; hay que seleccionarlo o el guardado falla. |
...
otros, el de bienes corrientes, el de bienes de inversión, el de servicios corrientes y las
variantes intracomunitaria, de importación y de inversión del sujeto pasivo.
...
En cada **sincronización de productos** —la acción planificada nº 2, cada dos horas— el conector
recorre todas las categorías de OkTicket y escribe en el producto las filas que la normativa
española admite para esa categoría. No hay nada que ejecutar a mano ni ningún dato que mantener al
día: la tabla de criterios vive en el código del módulo, así que se actualiza al actualizar el
módulo.

Sobre la base de referencia son **22 filas por compañía**, sin configuración previa.

Tres reglas gobiernan la escritura:

| Regla | Qué significa |
|---|---|
| **Solo crea lo que falta** | Una fila que ya existe no se toca nunca, la haya puesto una persona o una ejecución anterior. |
| **Lo editado es del cliente** | Al cambiar el impuesto de una fila propuesta, la marca *Propuesto por el conector* desaparece y esa fila queda excluida de futuras actualizaciones. |
| **Nunca inventa** | Si el plan contable no está cargado o no existe el impuesto, la fila no se crea y la sincronización continúa. |

La casilla **Propuesto por el conector** de cada fila es la que distingue lo escrito por el módulo
de lo revisado por una persona. Es de solo lectura.

### Qué criterios aplica

Los de la Ley 37/1992, categoría por categoría. Las catorce categorías por defecto de OkTicket:

| Categoría | Naturaleza | Tipos que se declaran | Base legal |
|---|---|---|---|
| Restauración | servicio | 10 % servicios · 21 % bienes · 4 % bienes | 91.Uno.2.2º hostelería y consumo en el acto · 90.Uno alcohol y refrescos azucarados como bien · 91.Dos.1.1º alimentos básicos |
| Alojamiento | servicio | 10 % · 21 % | 91.Uno.2.2º · 90.Uno para extras no hosteleros |
| Transporte, Bus, VTC, Taxi, Tren | servicio | 10 % | 91.Uno.2.1º transporte de viajeros |
| Avión | servicio | 10 % · 0 % exento | 91.Uno.2.1º · 22.Trece internacional aéreo |
| Tren | servicio | 10 % · 0 % **no sujeto** | 70.Uno.2º: el 22.Trece no alcanza al ferrocarril |
| Otros Transportes | servicio | 10 % · 21 % · 0 % exento | viajeros · mercancías · 21.5º, 23 y 24 |
| Peaje, Aparcamiento, Alquiler vehículo | servicio | 21 % | 90.Uno; 11.Dos.2º para el aparcamiento y el alquiler |
| **Gasolina** | **bienes** | 21 % · 10 % | 90.Uno · art. 42 del RDL 7/2026, ventana 22/03–30/06/2026 |
| Otros | — | *ninguno* | Cajón de sastre: no hay naturaleza que afirmar |

Dos detalles que merecen atención:

- **Gasolina es la única categoría de bienes**, y es justo la que el conector no resuelve solo en
  ninguna versión: bienes corrientes y bienes de inversión comparten tipo, ámbito y condición de
  base. Sin esta tabla, los repostajes entraban sin IVA.
- **El 0 % tiene tres significados** —exento con derecho (art. 22 y 21.5º/23/24), no sujeto
  (art. 70.Uno.2º) y exento de operaciones corrientes— y el plan trae cinco impuestos base al 0 %.
  Por eso un vuelo internacional y un trayecto de tren transfronterizo no apuntan al mismo impuesto.
...
Por el **nombre**, con el identificador de OkTicket corroborando. Las catorce categorías por defecto
no llevan compañía y parecen un catálogo compartido por todos los clientes, pero eso está observado
en un único entorno. Si el identificador no coincide con el esperado, la fila se escribe igual y
queda un aviso en el log; si lo que no coincide es el nombre, **no se escribe nada** y el conector
se comporta como siempre.

La regla de fondo: la tabla solo puede mejorar el resultado, nunca empeorarlo.

### La tabla en la ficha del producto
...
| *Propuesto por el conector* | Marcado si lo escribió el módulo. Al cambiar el impuesto se desmarca solo. |
| *Compañía* | Oculta por defecto. Los impuestos son por compañía, así que en multi-compañía hay una fila por cada una; la regla de registro deja a la vista solo las de las compañías activas. |
...
| El backend no se guarda y el campo *Versión* aparece en rojo. | *Versión* es obligatorio y no viene preseleccionado. | Seleccionar *1.0 y superior* y guardar. |
...
<p align="center">
  ...
  <sub>Conector OkTicket para Odoo 16 · Licencia AGPL-3</sub>
</p>
