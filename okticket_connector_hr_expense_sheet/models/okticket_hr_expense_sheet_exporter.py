...
class HrExpenseExporter(Component):
    ...
    def generate_new_expense_sheet(self, expense_sheet, backend_adapter):
        ...
        while found_exp_sheets:
            ...
            if found_exp_sheets:
                new_name_to_test = new_name_to_test + ' | ' + str(index + 1)
                index += 1
            max_tries -= 1
            if max_tries == 0:
                new_name_to_test = expense_sheet.name + '-' + date_str
    ...
    def run(self, *args):
        ...
        if expense_sheet and self.work.collection.okticket_exp_sheet_sync:  # Habilitada la sincronización de hojas en okticket
